% Fig. 4 - Proposed Scheme (parallel arrows, wider spacing 0.7, unified with Fig 3)
% Compile: pdflatex fig4_new_msc.tex
...
% ============================================================
% STEP 1: MSG1 (same parallel pattern as Fig 3, spacing 0.7)
% ============================================================
\node[phase] at (-2.0, -1.0) {Paso 1};

\draw[msg] (0, -1.5) -- node[sloped, above, font=\small, pos=0.55]
    {MSG1: Preamble (RAPID $r$)} (10, -2.5);
\draw[msg] (2.5, -2.45) -- (10, -3.2);
\draw[msg] (5, -3.4) -- (10, -3.9);

\node[sidenote, anchor=north east] at (-0.3, -1.5)
    {Tres UEs seleccionan\\
     el mismo preámbulo. Cada\\
     UE registra su \textit{timing advance}\\
     en el instante de transmisión de MSG1};

\node[callout, anchor=north west] at (10.5, -2.5) {
    \textbf{Evento en eNB: detección de colisión + agrupamiento espacial}\\
    Detección probabilística de colisión;
    los UEs colisionantes se agrupan
    por tiempo de llegada (ToA).
};

% ============================================================
% STEP 2: MSG2 (same parallel pattern, spacing 0.7)
% ============================================================
\node[phase] at (-2.0, -7.3) {Paso 2};

\node[callout, anchor=north west] at (10.5, -7.1) {
    \textbf{Preparación de MSG2 en eNB}\\
    Emite una RAR de clúster con TC-RNTI
    compartido, codebook SCMA asignado,
    UL Grant y referencia de \textit{timing advance}
    del clúster.
};

\draw[msg] (10, -9.0) -- node[sloped, above, font=\small, pos=0.45]
    {MSG2: RAR de clúster} (0, -10.0);
\draw[msg] (10, -9.7) -- (2.5, -10.45);
\draw[msg] (10, -10.4) -- (5, -10.9);

\node[sidenote, anchor=north east] at (-0.3, -10.3)
    {Cada UE verifica\\
     pertenencia al clúster\\
     comparando su \textit{timing advance}\\
     con la referencia};

\node[sidenote, anchor=north] at (2.5, -10.8)
    {Los UEs del clúster\\
     comparten TC-RNTI\\
     y capa SCMA determinista};

% ============================================================
% STEP 3: MSG3 (same parallel pattern, spacing 0.7, pos staggered)
% ============================================================
\node[phase] at (-2.0, -14.0) {Paso 3};

\node[sidenote, anchor=north east] at (-0.3, -14.3)
    {Cada UE envía MSG3 sobre el\\
     codebook del clúster eligiendo un piloto\\
     al azar del pool, e incluye su IMSI};
...
\node[callout, anchor=north west] at (10.5, -16.5) {
    \textbf{Evento en eNB: decodificación ciega por MPA}\\
    Separación por pilotos dentro del
    clúster; colisión de piloto si coinciden.
};

% ============================================================
% Outcomes (same parallel pattern, spacing 0.7)
% ============================================================
...
\node[callout, anchor=north west] at (10.5, -23.8) {
    \textbf{Preparación de MSG4 en eNB}\\
    Individualización del C-RNTI mediante IMSI
    sobre el TC-RNTI compartido; los UEs
    no decodificados reintentan tras backoff.
};

\draw[msg] (10, -25.5) -- node[sloped, above, font=\small]
    {MSG4: (TC-RNTI compartido, IMSI, C-RNTI)} (2.5, -26.25);
