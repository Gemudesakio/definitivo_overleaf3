...
\node[sidenote, anchor=north east, text width=3.6cm] at (-0.3, -1.5)
...
\node[sidenote, align=center, text width=4.8cm] at (-2.5, -11)
{
Cada UE selecciona aleatoriamente una RAR
del conjunto y adopta el TC-RNTI asociado.
};
...
\node[sidenote, anchor=north east, text width=4.4cm] at (-0.3, -14.5)
    {Cada UE elige al azar una capa SCMA y un piloto del pool, y envía MSG3 con su identidad de UE (IMSI)};
...
\node[sidenote, anchor=north east, text width=3.6cm] at (-0.3, -26.1)
    {UE$_1$: sin respuesta\\
     $\rightarrow$ backoff y reintenta};

\node[sidenote, anchor=north west, text width=3.6cm] at (5.3, -26.1)
    {UE$_3$: sin respuesta\\
     $\rightarrow$ backoff y reintenta};
...
\node[font=\small\bfseries, text=green!40!black, align=center, text width=10cm] at (5, -30.7)
    {UE$_2$ completa el acceso aleatorio; UE$_1$ y UE$_3$ reintentan por colisión de piloto};
