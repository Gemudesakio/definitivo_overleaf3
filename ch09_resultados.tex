...
Este capítulo presenta y discute los hallazgos empíricos de la evaluación de los dos esquemas propuestos en el \cref{ch:diseno} (CRACS-R y CRACS-T) frente al baseline Legacy del estándar NB-IoT \cite{ts36331}. La campaña experimental siguió el plan metodológico descrito en la \cref{sec:metodologia}, implementado sobre el simulador descrito en el \cref{ch:implementacion}, y consistió en $4\,260$ corridas independientes: $3\,990$ de la campaña principal sobre los nueve escenarios E0--E6\_burst, y $270$ del escenario adicional E7\_t300 que caracterizó la sensibilidad al temporizador $T_{300}$.
...
\]
y con los cinco criterios cuantitativos de aceptación C1--C5 (\cref{sec:design_criterios}). Como se mostrará, la conjetura ex-ante se cumple en $38$ de los $42$ puntos operativos evaluados (criterio: $\mathrm{SAR}_{\text{Legacy}} \leq \mathrm{SAR}_{\text{CRACS-R}} \leq \mathrm{SAR}_{\text{CRACS-T}}$ aplicado sobre la media muestral de SAR en cada combinación de escenario y valor barrido), pero \emph{deja de cumplirse en una región importante en la práctica}: para que CRACS-T supere a CRACS-R hace falta $T_{300} \geq 10$~s; con el valor por defecto del estándar ($T_{300}=5$~s, recomendado por 3GPP TS~36.331), CRACS-R supera a CRACS-T en $\mathrm{SAR}$ a $K{=}300$ ráfaga. Esta dependencia solo apareció en los datos y se trata como un requisito de despliegue de CRACS-T en \cref{sec:res_sintesis} y como una limitación en \cref{sec:res_limitaciones}.

La organización es la siguiente. La \cref{sec:res_config} resume la configuración experimental y el método estadístico. La \cref{sec:res_e0} valida la coherencia del simulador con el escenario de un solo UE. Las \cref{sec:res_e1,sec:res_e4,sec:res_e5,sec:res_e6} reportan los escenarios de carga, calibración, robustez al pool NPRACH y saturación, respectivamente. La \cref{sec:res_e7} expone el hallazgo central sobre $T_{300}$, incluida una prueba de control que confirma que el efecto se debe al temporizador. La \cref{sec:res_criterios} contrasta los resultados con los criterios C1--C5. Finalmente, las \cref{sec:res_sintesis,sec:res_limitaciones,sec:res_cierre} discuten implicaciones, limitaciones y cierre.

\section{Configuración experimental y método estadístico}
\label{sec:res_config}

La \cref{tab:res_config} sintetiza los diez escenarios ejecutados, ya descritos en detalle en los \cref{sec:metodologia,ch:implementacion}. Las cinco subsecciones siguientes formalizan las elecciones metodológicas que sustentan los resultados reportados a lo largo del capítulo.

\begin{table}[!t]
\centering
\caption{Síntesis de escenarios ejecutados.}
\label{tab:res_config}
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}l l l l c@{}}
\toprule
\textbf{Esc.} & \textbf{Variable barrida} & \textbf{Rango} & \textbf{Esquemas} & \textbf{Corridas} \\
\midrule
E0          & $K$            & 1               & 3 & 90  \\
E1          & $K$ (avg)      & 1--100 (11 pts) & 3 & 990 \\
E1\_high    & $K$ (high)     & 1--100 (11 pts) & 3 & 990 \\
E4          & $\Delta d_{\min}$ (avg)  & 50--1000 m (8 pts) & CRACS-T & 240 \\
E4\_high    & $\Delta d_{\min}$ (high) & 50--1000 m (8 pts) & CRACS-T & 240 \\
E5          & $N_{\text{pream}}$ (avg) & 12--48 (4 pts) & 3 & 360 \\
E5\_high    & $N_{\text{pream}}$ (high)& 12--48 (4 pts) & 3 & 360 \\
E6\_uniform & $K$ (Poisson TM1) & 30--1000 (4 pts) & 3 & 360 \\
E6\_burst   & $K$ (Beta TM2)    & 30--1000 (4 pts) & 3 & 360 \\
E7\_t300    & $T_{300}$ (en E6\_burst $K{=}300$) & 5, 10, 40 s & 3 & 270 \\
\midrule
\multicolumn{4}{r}{\textbf{Total}} & $4\,260$ \\
\bottomrule
\end{tabular}
\end{table}

El valor de referencia del timer fue $T_{300}=5$~s, valor recomendado por el estándar NB-IoT \cite{ts36331}, que pertenece al subconjunto $\{2{,}5,\, 4,\, 5,\, 6,\, 8,\, 10,\, 15,\, 25,\, 40,\, 60\}$~s admitido por el estándar. La duración simulada por corrida fue $T_{\text{sim}}=120$~s (180~s en E1, E1\_high, E4\_high y E5\_high) sobre un único satélite LEO a $h_{\text{sat}}=600$~km con $\alpha_{\min}=40^\circ$ y los demás parámetros listados en la \cref{tab:impl_flags}.

\subsection{Tamaño de muestra y régimen de simulación}
\label{sec:res_config_n}

La literatura de comunicaciones inalámbricas distingue dos regímenes de \emph{simulación de Monte Carlo}: (i) el régimen \emph{analítico}, en el que cada muestra es un sorteo aleatorio rápido de un modelo cerrado (típicamente $10^{4}$--$10^{6}$ muestras por punto, costo $\sim$\,$\mu$s); y (ii) el régimen \emph{system-level}, en el que cada muestra es una corrida completa del simulador que reproduce el procedimiento de acceso aleatorio, las retransmisiones, los temporizadores y el modelo PHY (típicamente $20$--$50$ semillas por punto, costo $\sim$\,segundos a minutos) \cite{ns3, kodheli2021nbiotntnra}. Las $4\,260$ corridas de este trabajo pertenecen al segundo régimen: cada corrida ejecutó $120$--$180$~s de tiempo simulado sobre ns-3.32 con $K$ hasta $1\,000$ UEs activos, con un tiempo de cómputo del orden de segundos a decenas de segundos por corrida. Bajo este costo, $n=30$ semillas por punto satisface la convergencia empírica de la media (\cref{fig:res_convergence}) y se ubica dentro del rango habitual de los estudios system-level de NB-IoT y NORA \cite{wang2020nbnora, sanchez2019nbiotra}; un objetivo de $n=10^{4}$ por punto correspondería al régimen analítico y requeriría reformular el experimento.

\begin{figure}[!t]
\centering
\includegraphics[width=\linewidth]{fig912_convergence.pdf}
\caption{Convergencia de la media de SAR como función del número acumulado de semillas para dos casos representativos seleccionados por tener varianza inter-semilla no nula: (a) Legacy bajo régimen E1 $K{=}100$ (perfil avg, $\bar{x}=0{,}903$, $s=0{,}076$) y (b) CRACS-R bajo régimen E6\_burst $K{=}300$ (perfil ráfaga, $\bar{x}=0{,}783$, $s=0{,}096$). La banda sombreada indica el IC al $95\%$ acumulado y la línea punteada gris la asíntota de la media. Ambos casos estabilizan dentro del $2\%$ de su valor final antes de la semilla $20$, lo que justifica $n=30$ como tamaño de muestra suficiente. Los casos con SAR saturada a $1{,}000$ (e.g.\ CRACS-T en E1\_high $K{=}100$) producen $s=0$ y curva plana sin banda IC, por lo que no aportan información sobre la convergencia.}
\label{fig:res_convergence}
\end{figure}

La curva de convergencia muestra que el ancho del IC se reduce a aproximadamente $1/\sqrt{n}$ y que, en el régimen estacionario, la media muestral acumulada cruza dentro del $2\%$ de su valor asintótico antes de la semilla $20$. Las $10$ semillas restantes refuerzan la estabilidad sin alterar las conclusiones cualitativas. Aumentar $n$ a $100$ reduciría el ancho del IC en un factor de aproximadamente $1{,}8\times$ a costa de triplicar el tiempo de cómputo, sin modificar las conclusiones reportadas.

\subsection{Intervalos de confianza al 95\,\%}
\label{sec:res_config_ci}

Para cada celda (escenario, esquema, valor barrido) se reporta la media muestral acompañada de su IC al $95\%$ calculado con la distribución $t$-Student y $\nu = 29$ grados de libertad \cite{walpole2012probability},
\[
\mathrm{IC}_{95\%}(\bar{x}) = \bar{x} \pm t_{0{,}025,\,29}\,\frac{s}{\sqrt{n}},
\]
donde $s$ es la desviación estándar muestral. La aproximación es válida en virtud del Teorema del Límite Central, que aplica a la media de las $n=30$ muestras independientes incluso cuando la distribución subyacente del KPI no es gaussiana. La \cref{sec:res_config_outliers} reporta además un IC bootstrap percentile como verificación independiente.

\subsection{Estadísticos robustos a outliers}
\label{sec:res_config_robust}

Las métricas de latencia ($T_{\text{access}}$, $T_{\text{inst}}$, TST) presentan distribuciones con cola larga: unos pocos UEs sufren múltiples reintentos y producen valores muy por encima del promedio. La media muestral es sensible a esta cola; la mediana y los percentiles no \cite{wilcoxon1945}. Por ello $T_{\text{access}}$ se reporta simultáneamente como media, mediana ($p50$) y percentil $95$ ($p95$): la media captura el agregado, la mediana captura el comportamiento típico y el $p95$ acota el caso peor de la cola. Adicionalmente, el SDP(T) (probabilidad de éxito antes de $T$~s, normalizada por el total de UEs) y la TST (\emph{makespan} de la oleada) son métricas distribucionales que son por construcción robustas a outliers individuales. Las curvas de $\mathrm{SDP}(T)$ vs.\ $K$ (\cref{fig:res_sdp}) y los diagramas de caja de $T_{\text{access}}$ por punto operativo (\cref{fig:res_box_taccess} y siguientes) reportan la distribución de cada KPI más allá del estadístico de resumen único.

\subsection{Comparación entre esquemas: prueba de Wilcoxon}
\label{sec:res_config_wilcoxon}

Las diferencias entre esquemas se contrastan con la prueba de Wilcoxon de rangos con signo \cite{wilcoxon1945}, una prueba no paramétrica que no requiere normalidad de los datos y que es robusta frente a outliers. Se reporta una diferencia como \emph{estadísticamente significativa} cuando $p<0{,}05$. La elección de Wilcoxon sobre $t$-pareado se justifica por dos razones: (i) varias métricas saturan cerca de $1{,}0$ (SAR en regímenes moderados), lo que produce distribuciones de diferencias claramente no gaussianas; (ii) las pruebas no paramétricas son la convención en estudios system-level de NB-IoT \cite{sanchez2019nbiotra, harwahyu2019powerra}.

\subsection{Outliers y verificación de robustez}
\label{sec:res_config_outliers}

Para cuantificar la sensibilidad a outliers se aplica la regla de Tukey $1{,}5\cdot$IQR sobre las $30$ muestras por celda: una observación se clasifica como outlier si cae fuera del intervalo $[Q_1 - 1{,}5\cdot\text{IQR},\, Q_3 + 1{,}5\cdot\text{IQR}]$, criterio idéntico al empleado por el diagrama de cajas estándar de MATLAB y matplotlib. El análisis completo se detalla en la \cref{sec:res_e1_outliers} (\cref{tab:res_outliers}); brevemente: los outliers detectados corresponden mayoritariamente a UEs que sufrieron varios reintentos consecutivos. Son \emph{datos reales} que representan la cola legítima del proceso de acceso, no errores de medición; por ello \emph{se conservan} en el análisis y se reportan por transparencia.
...
Los escenarios E1 y E1\_high barren la carga $K \in \{1, 5, 10, 15, 20, 25, 30, 40, 50, 75, 100\}$ bajo perfil de tráfico moderado (avg, $\lambda \approx 7{,}3$~UEs/s) e intenso (high, $\lambda \approx 27{,}8$~UEs/s) respectivamente. La \cref{fig:res_sar} compara la SAR de los tres esquemas en ambos regímenes; la \cref{fig:res_taccess} reporta la latencia media de acceso $T_{\text{access}}$ y la \cref{fig:res_sdp} la métrica conjunta latencia--confiabilidad $\mathrm{SDP}(3\text{~s})$ (probabilidad de éxito antes de 3~s sobre el \emph{total} de UEs).

\begin{figure}[!t]
\centering
\includegraphics[width=\linewidth]{fig91_sar_vs_k.pdf}
\caption{Tasa de acceso exitoso ($\mathrm{SAR}$) en función de la carga $K$ para los tres esquemas bajo perfil de tráfico moderado (a) e intenso (b). Las barras de error muestran el intervalo de confianza al $95\%$ sobre $n=30$ semillas.}
\label{fig:res_sar}
\end{figure}

Bajo perfil avg la SAR de Legacy se mantuvo $\geq 0{,}999$ hasta $K=50$ ($0{,}9987$) y descendió a $0{,}965$ a $K=75$ y $0{,}903$ a $K=100$. CRACS-R y CRACS-T conservaron $\mathrm{SAR} \approx 1{,}000$ en todo el rango (CRACS-R: $0{,}996$ a $K=100$). Bajo perfil high la SAR de Legacy descendió antes y con mayor magnitud absoluta, cayendo a $0{,}825$ a $K=100$ ($\Delta = -0{,}078$ frente al perfil avg al mismo $K$); CRACS-R y CRACS-T se mantuvieron ambos en $\mathrm{SAR}=1{,}000$ a $K=100$. En este régimen la SAR no separa CRACS-R de CRACS-T: ambos saturan la métrica en su techo.

La separación entre CRACS-R y CRACS-T se hace visible en la \cref{fig:res_taccess}. A $K=100$ bajo perfil avg, $T_{\text{access}}$ de CRACS-T fue $638{,}4$~ms, frente a $2\,510{,}5$~ms de CRACS-R ($\times 3{,}9$~veces mayor) y $8\,651{,}6$~ms de Legacy ($\times 13{,}6$~veces mayor). Bajo perfil high a $K=100$, los valores fueron $2\,682{,}7$, $3\,531{,}3$ y $11\,961{,}7$~ms para CRACS-T, CRACS-R y Legacy respectivamente. La prueba de Wilcoxon entre CRACS-R y CRACS-T sobre $T_{\text{access}}$ bajo perfil high arrojó $p=4{,}66\times 10^{-3}$ a $K=100$ y $p=3{,}74\times 10^{-3}$ a $K=75$, ambos significativos.
...
La métrica $\mathrm{SDP}(3\text{~s})$ (\cref{fig:res_sdp}) cuantifica simultáneamente confiabilidad y latencia. A $K=100$ bajo avg, CRACS-T alcanzó $\mathrm{SDP}(3\text{~s})=0{,}994$ frente a $0{,}672$ de CRACS-R y $0{,}323$ de Legacy; bajo high los valores fueron $0{,}602$, $0{,}467$ y $0{,}185$. La \cref{tab:res_e1_k100} consolida los principales KPIs en el punto operativo $K=100$.
...
La conjetura de inclusión \emph{ex-ante} se cumple en este régimen: $\Omega_{\text{Legacy}} \subsetneq \Omega_{\text{CRACS-R}} \approx \Omega_{\text{CRACS-T}}$ en términos de SAR ($K \leq 100$), y la jerarquía se vuelve estricta en términos de $T_{\text{access}}$ y $\mathrm{SDP}$. Es decir, la separación CRACS-T sobre CRACS-R en esta franja de carga es de \emph{eficiencia de acceso}, no de \emph{capacidad bruta}: ambos esquemas admiten todos los UEs, pero CRACS-T lo hace al primer intento el $94{,}8\%$ de las veces frente al $44{,}4\%$ de CRACS-R, gracias a la asignación determinista de codebook SCMA por clúster.

\begin{figure}[!t]
\centering
\includegraphics[width=\linewidth]{fig99_boxplot_taccess_K100.pdf}
\caption{Distribución de $T_{\text{access}}$ medio a $K{=}100$ bajo perfil moderado (a) e intenso (b). Las cajas muestran $Q_1$--$Q_3$, la línea central la mediana, los bigotes el rango Tukey $1{,}5\cdot$IQR y las cruces rojas los outliers individuales (UEs con múltiples reintentos). Eje vertical logarítmico.}
\label{fig:res_box_taccess}
\end{figure}

La \cref{fig:res_box_taccess} complementa la \cref{fig:res_taccess} mostrando la distribución completa de las 30 muestras por esquema. Se desprenden tres observaciones cuantitativas: (i) la mediana de CRACS-T es $3{,}6\times$ menor que la de CRACS-R y $11{,}7\times$ menor que la de Legacy bajo perfil avg, y $1{,}7\times$ menor que CRACS-R y $5{,}6\times$ menor que Legacy bajo perfil high (medianas calculadas sobre las 30 semillas a $K=100$); (ii) las cajas de Legacy presentan asimetría hacia arriba (cola larga superior) consistente con la distancia entre su media y su mediana reportada en \cref{tab:res_e1_k100}; (iii) los outliers visibles en Legacy y, en menor medida, en CRACS-R caen por encima del cuartil superior, comportamiento consistente con corridas donde un subconjunto de UEs requirió retransmisiones adicionales, aunque la atribución a UE individual con múltiples reintentos no se verifica directamente a nivel de trayectoria por UE en esta lectura agregada.
...
\midrule
E1 $K{=}100$ & $T_{\text{access}}$ (ms) & Legacy & 30 & 0 & 0.0 & 8651.6 & 8651.6 & $+0{,}0$ \\
 & & CRACS-R & 30 & 1 & 3.3 & 2510.5 & 2429.9 & $-3{,}2$ \\
 & & CRACS-T & 30 & 1 & 3.3 & 638.4 & 630.9 & $-1{,}2$ \\
\addlinespace[2pt]
E1\_high $K{=}100$ & $T_{\text{access}}$ (ms) & Legacy & 30 & 0 & 0.0 & 11961.7 & 11961.7 & $+0{,}0$ \\
 & & CRACS-R & 30 & 0 & 0.0 & 3531.3 & 3531.3 & $+0{,}0$ \\
 & & CRACS-T & 30 & 0 & 0.0 & 2682.7 & 2682.7 & $+0{,}0$ \\
\addlinespace[2pt]
E6\_burst $K{=}300$ & $T_{\text{access}}$ (ms) & Legacy & 30 & 3 & 10.0 & 28359.8 & 28612.5 & $+0{,}9$ \\
 & & CRACS-R & 30 & 1 & 3.3 & 6671.0 & 6789.2 & $+1{,}8$ \\
 & & CRACS-T & 30 & 0 & 0.0 & 5311.3 & 5311.3 & $+0{,}0$ \\
\addlinespace[2pt]
E6\_uniform $K{=}1000$ & $T_{\text{access}}$ (ms) & Legacy & 30 & 0 & 0.0 & 8031.9 & 8031.9 & $+0{,}0$ \\
 & & CRACS-R & 30 & 0 & 0.0 & 5511.9 & 5511.9 & $+0{,}0$ \\
 & & CRACS-T & 30 & 0 & 0.0 & 5711.5 & 5711.5 & $+0{,}0$ \\
\addlinespace[2pt]
...
Tres observaciones se desprenden de la \cref{tab:res_outliers}. Primero, la tasa de outliers fue baja: el máximo observado fue $10{,}0\%$ (Legacy en E6\_burst $K{=}300$ y en E7\_t300 $T_{300}{=}40$~s); la mayoría de las celdas tienen cero outliers. Segundo, los outliers se concentran en Legacy (esquema con la cola más larga observada en el barrido, consistente con la ausencia de multicapa SCMA aunque no medida específicamente sobre los UEs outlier) y son prácticamente nulos en CRACS-R/CRACS-T para SAR. Tercero, y más importante: el cambio relativo de la media al excluir los outliers ($\Delta\%$) se mantuvo por debajo del $\pm 3{,}5\%$ en todos los casos, y las conclusiones cualitativas (CRACS-T $\ll$ Legacy en latencia, CRACS-R $>$ CRACS-T en SAR a saturación) son idénticas al excluirlos.

Los outliers se conservaron en el análisis principal porque representan UEs reales del proceso de acceso aleatorio que sufrieron varios reintentos hasta el éxito; descartarlos sesgaría el comportamiento del sistema en su cola, justamente la zona donde las propuestas se diferencian del baseline.
...
En todo el rango probado el número medio de clústeres se mantuvo cerca o en el techo $K_{\text{CB}}=6$ ($5{,}67$--$5{,}77$ en avg; $6{,}00$ exacto en high) y la SAR fue $1{,}000$ en todos los puntos. El parámetro $\Delta d_{\min}$ no resultó ser sensible en el rango $[50, 1000]$~m: la dispersión diferencial $\Delta d$ dentro de la celda satura el techo de codebooks en todos los valores probados, lo cual explica la meseta observada. Esto valida empíricamente el valor por defecto $\Delta d_{\min}=150$~m elegido en el \cref{ch:diseno} y muestra que el clustering ToA de CRACS-T es robusto a esta calibración dentro del rango probado $[50, 1000]$~m (un factor de $20\times$, equivalente a $\approx 1{,}3$ órdenes de magnitud); el comportamiento fuera de ese rango no se caracterizó. El criterio C3 (precisión de agrupamiento) se cumple holgadamente: la tasa de identificación incorrecta de clúster (CID) se mantuvo por debajo del $3{,}5\%$ en todo el barrido.
...
La inversión observada en \cref{sec:res_e6} bajo saturación motivó un experimento dedicado: caracterizar la sensibilidad del trade-off al temporizador RRC $T_{300}$, parámetro que define el plazo máximo del que dispone el UE para completar el procedimiento de acceso antes de abandonarlo \cite{ts36331}.
...
E7\_t300 barrió $T_{300} \in \{5, 10, 40\}$~s --- tres valores del subconjunto admitido por \cite{ts36331} listado en \cref{sec:res_config} --- manteniendo el resto de parámetros idénticos a E6\_burst $K=300$. La \cref{fig:res_t300} reporta la SAR de los tres esquemas en función de $T_{300}$.

\begin{figure}[!t]
\centering
\includegraphics[width=0.75\linewidth]{fig98_t300_sweep.pdf}
\caption{Tasa de acceso exitoso en función del temporizador $T_{300}$ (escenario E6\_burst $K=300$). La región sombreada delimita el rango estándar NB-IoT \cite{ts36331}.}
\label{fig:res_t300}
\end{figure}
...
La \cref{tab:res_criterios} contrasta los cinco criterios C1--C5 de la \cref{sec:design_criterios} con los hallazgos empíricos.
...
\midrule
C1 & Cumple cond. & $\mathrm{SAR}_{\text{CRACS-T}} > \mathrm{SAR}_{\text{CRACS-R}}$ se cumple para $K \leq 100$ bajo avg/high y para $T_{300} \geq 10$~s en saturación. \emph{No se cumple} bajo $T_{300}=5$~s a $K \geq 300$ ráfaga, valor que coincide con el por defecto del estándar (\cref{sec:res_e6,sec:res_e7}). \\
