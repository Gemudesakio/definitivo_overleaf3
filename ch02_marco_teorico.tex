...
tecnologías anteriores~\cite{ref3, ref12}.
...
uno de tres niveles de cobertura: CE0, CE1 y CE2~\cite{ref4}. Esta clasificación define
...
ortogonales por oportunidad de acceso~\cite{ref4}. Este número, que ya es bajo para escenarios
...
dispositivo y la red~\cite{ref6, ref4}. La Figura~\ref{fig:ra_msc} resume ese intercambio,
...
establecido el control de recursos de radio (RRC, \textit{Radio Resource Control}).
Por último, el UE confirma con un
acuse de recibo (MSG5), y a partir de ese momento la conexión queda
establecida~\cite{ref4}.
...
deben esperar a que expire el temporizador T300 y empezar de nuevo desde MSG1~\cite{ref5,
ref4}. En los niveles CE1 y CE2, donde las oportunidades de acceso pueden estar separadas por
...
Ecuación~\ref{eq:pcol}~\cite{ref5}.
...
parte de los intentos de acceso fallen en el primer mensaje~\cite{ref4}.
...
cada intervalo, reduciendo la congestión pero aumentando el retardo~\cite{ref6}. La segunda,
...
conexión sin usar recursos adicionales~\cite{ref5, ref6}. La base técnica de este enfoque es
...
\textit{Successive Interference Cancellation}) en el receptor~\cite{ref6, ref13}. En el
...
mensaje de forma independiente~\cite{ref5, ref6}.

El impacto de esta modificación en el desempeño es importante. Liang
\textit{et al.}~\cite{ref5} demuestran que NORA supera al esquema ortogonal convencional en
...
Por su parte, Seo \textit{et al.}~\cite{ref6} muestran que el \textit{throughput} máximo con
...
colisiones y eleva el \textit{throughput} máximo respecto al ALOHA puro.} convencional. Wang \textit{et al.}~\cite{ref13} amplían este análisis
...
de los usuarios~\cite{ref14}. El NORA triangular (T-NORA, \textit{Triangular NORA}) adapta el
procedimiento a paquetes de distinto tamaño en sistemas de antenas masivas~\cite{ref15},
mientras que el NORA con paginación grupal (NORA-GP, \textit{NORA-based Group Paging})
controla la sobrecarga en redes 5G con tráfico MTC intenso~\cite{ref16}. En el caso
particular de NB-IoT, Wang \textit{et al.}~\cite{ref4} proponen el NORA para NB-IoT
...
\textit{Low-Density Code-Domain NOMA})~\cite{ref9, ref10}. Su idea principal es asignar a
...
uno usa un patrón de subportadoras distinto~\cite{ref9, ref10}. Por ejemplo, si hay cuatro
...
con uno del otro y nunca dos nodos del mismo conjunto.}~\cite{ref9, ref10}. En ese grafo
...
máxima verosimilitud~\cite{ref10}. A diferencia del SIC, el MPA no necesita que las señales tengan
niveles de potencia diferentes, lo que lo hace más robusto cuando los dispositivos están a
distancias parecidas del eNB~\cite{ref9}.
...
su MSG3~\cite{ref9}. El detector MPA en recepción procesa los MSG3 recibidos sobre los
...
con la secuencia Zadoff-Chu generada en el eNB~\cite{ref11}. Cuando un solo
...
indica que hubo una colisión~\cite{ref11}.

Estudios recientes han mostrado que las técnicas de aprendizaje automático pueden mejorar
mucho la precisión de esta detección. Maldonado Cardenas \textit{et al.}~\cite{ref11}
...
\begin{itemize}
    ...
    \item \textbf{Especificidad} (\textit{Specificity}): fracción de no-colisiones que el
          ...
          desperdicia recursos que se habrían usado correctamente~\cite{ref11}.
          \begin{equation}
...
existe~\cite{ref3, ref7}. Entre las distintas órbitas posibles, los satélites de órbita
...
35\,786 km~\cite{ref8, ref12}. Esta menor altitud ofrece dos ventajas principales: una
...
los aproximadamente 500 ms de la órbita geoestacionaria~\cite{ref8}. Sin embargo, la baja
...
quince minutos, dependiendo de la altitud y la elevación mínima~\cite{ref12}. Esto tiene
...
con las redes terrestres~\cite{ref7}.
...
caso~\cite{ref8}. Este desplazamiento afecta la ortogonalidad entre subportadoras en sistemas
...
pueden llegar a varios kilohertz y deben manejarse en el receptor~\cite{ref8}. También se debe
...
NB-IoT~\cite{ref7}. La Release 18 identifica la congestión en el acceso como un problema
abierto en escenarios NTN-IoT y plantea soluciones para versiones futuras~\cite{ref7}. Esta
...
de la capacidad del canal de acceso no ortogonal~\cite{ref5, ref8, ref7}. El comportamiento
...
probabilidad de error muy pequeña sobre un canal dado~\cite{ref17}. Mientras que la capacidad
...
operación del sistema~\cite{ref5, ref6, ref9}. Es la métrica adecuada porque en escenarios
...
de preámbulo y las fallas al decodificar MSG3~\cite{ref5, ref9}. El
...
NPRACH, y se vuelve especialmente crítico en los niveles CE1 y CE2~\cite{ref4}. Por último, la
...
en una ventana de tiempo dada sin que el sistema se sature~\cite{ref5}. Esta cantidad depende
...
modelos terrestres~\cite{ref8, ref7}.

La relación entre la densidad de dispositivos y la capacidad del canal se puede ver en la
curva de throughput del sistema. A medida que el número de dispositivos activos $M$
crece, el throughput aumenta al principio porque hay más transmisiones útiles, pero
a partir de cierto punto las colisiones empiezan a dominar y el throughput comienza
a bajar, hasta que el sistema colapsa por congestión~\cite{ref6}. El punto de máximo
throughput define la carga óptima de operación y, junto con la probabilidad de éxito
